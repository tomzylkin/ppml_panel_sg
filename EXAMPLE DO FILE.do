...
cap egen imp_time = group(isoimp year)
cap egen pair     = group(isoexp isoimp)
xi: glm trade i.exp_time i.imp_time ln_dist colony contig comlang_off comleg fta if year == 2000, diff iter(25) family(poisson) ro

* with the original results from lines 69 and 75, which only used the "TOTAL" category.
