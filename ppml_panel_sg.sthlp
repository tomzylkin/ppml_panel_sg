...
time trends.{p_end}
{p2col 8 12 12 2: 4.}All fixed effects are also allowed to vary by industry, in case you wish to examine industry-level variation
in trade flows.{p_end}
{p2col 8 12 12 2: 5.}Can be used with strictly cross-sectional regressions (i.e., without pair fixed effects).{p_end}
{p2col 8 12 12 2: 6.}Performs Santos Silva & Tenreyro (2010)'s recommended check for possible non-existence 
of estimates.{p_end}
...
{it:exp_id-imp_id[-ind_id]}, unless nopair is enabled.{p_end}
...
{synopt: {opt guessTT(varname)}}Guess initial values for pair fixed effects. These are not exponentiated. Default is all 0s.{p_end}

{pstd}{opth genD(newvar)}, {opth genS(newvar)}, {opth genM(newvar)}, {opth genO(newvar)}, {opth genI(newvar)}, and {opth genTT(newvar)}: These options store fixed effects and/or time trend parameters in memory as new variables.{p_end}
...
{pstd}This is version 1.0 of this command. If you believe you have found an error that can be replicated, or have other
...
Email: {browse "mailto:ecsztb@nus.edu.sg":ecsztb@nus.edu.sg}
{p_end}
...
each of these authors for creating these programs and making their code available.{p_end}
